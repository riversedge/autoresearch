...
from prepare import MAX_SEQ_LEN, TIME_BUDGET, EVAL_TOKENS, Tokenizer, evaluate_bpb, make_dataloader
...
class AdamW:
    def __init__(self, model, unembedding_lr, embedding_lr, matrix_lr, weight_decay, adam_betas, scalar_lr):
        ...
        for path, param in flat_params:
            if "blocks" in path and param.ndim == 2:
                self.param_config[path] = dict(lr=matrix_lr, betas=adam_betas, eps=1e-10, weight_decay=weight_decay)
            elif "wte" in path or "value_embeds" in path:
                self.param_config[path] = dict(lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0)
            elif "lm_head" in path:
                self.param_config[path] = dict(lr=unembedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0)
            elif "resid_lambdas" in path:
                self.param_config[path] = dict(lr=scalar_lr * 0.01, betas=adam_betas, eps=1e-10, weight_decay=0.0)
            elif "x0_lambdas" in path:
                self.param_config[path] = dict(lr=scalar_lr, betas=(0.96, 0.95), eps=1e-10, weight_decay=0.0)
            else:
                self.param_config[path] = dict(lr=unembedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0)
    ...
    def _step(self, path, grad, param, cfg):
    ...
    def set_lr_multiplier(self, multiplier):
        for path, cfg in self.param_config.items():
            cfg["lr"] = self.initial_lrs[path] * multiplier
...
# Optimization (AdamW-only for MLX)
...
# Model size tuned for 5-minute Apple Silicon loops
DEPTH = 4
...
PROFILES = {
    "mps_fast": dict(depth=4, window_pattern="SSSL", total_batch_size=2**16, device_batch_size=16),
    "mps_stable": dict(depth=4, window_pattern="L", total_batch_size=2**15, device_batch_size=8),
}
profile_name = os.getenv("AR_PROFILE", "mps_fast").lower()
if profile_name == "auto":
    profile_name = "mps_fast"
...
RUN_RESEARCH_EVAL = os.getenv("AR_RUN_RESEARCH_EVAL", "0") == "1"
...
loss_grad_fn = nn.value_and_grad(model, lambda m, inputs, targets: m(inputs, targets=targets))
...
loop_bpb = evaluate_bpb(
    model,
    tokenizer,
    FINAL_EVAL_BATCH_SIZE,
    backend="mlx",
    eval_tokens=LOOP_EVAL_TOKENS,
)
research_bpb = None
if RUN_RESEARCH_EVAL:
    research_bpb = evaluate_bpb(
        model,
        tokenizer,
        FINAL_EVAL_BATCH_SIZE,
        backend="mlx",
        eval_tokens=RESEARCH_EVAL_TOKENS,
    )
t_eval = time.time()
